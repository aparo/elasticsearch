...
public class TermsStatsLongFacetCollector extends AbstractFacetCollector {
    ...
    protected void doSetNextReader(AtomicReaderContext readerContext) throws IOException {
        keyFieldData = (NumericFieldData) fieldDataCache.cache(keyFieldDataType, readerContext.reader(), keyFieldName);
        if (script != null) {
            script.setNextReader(readerContext);
        } else {
            aggregator.valueFieldData = (NumericFieldData) fieldDataCache.cache(valueFieldDataType, readerContext.reader(), valueFieldName);
        }
    }
